    ...
    for equiv_class in equiv_classes:
        ...
        for i, observation in enumerate(observations):
            for entity in equiv_class:
                ...
                if list_of_gaps[i].intersection(entity) != set() and all(observation[j] for j in entity):
                    counter += 1
        number_of_appearances.append(counter)
    # 2nd step: retrieve the pattern that appeared the most
    maximum = max(number_of_appearances)
...
def find_basic_patterns(size, observations, trafos, output, basic_patterns):
    ...
    while True:
        ...
        equiv_classes = find_subsets(size, original_observation, basic_patterns, trafos, gaps)
